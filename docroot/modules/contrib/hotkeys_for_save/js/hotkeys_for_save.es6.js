...
(function ($, Drupal) {
  /**
   * Allow use hotkeys instead of clicking on 'Save' submit button.
   *
   * @type {Drupal~behavior}
   *
   * @prop {Drupal~behaviorAttach} attach
   *   Attaches functionality for handling keyboard events and clicking on submit buttons.
   */
  Drupal.behaviors.hotkeysForSave = {
    attach(context) {
      ...
      $(document, context).once().each((i, obj) => {
        $(obj).keyup((e) => {
          if (e.keyCode === 83) {
            clickAllowed = true;
          }
