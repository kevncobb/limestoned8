...
namespace Drupal\views_infinite_scroll_with_pager\EventSubscriber;
...
class AjaxResponseSubscriber implements EventSubscriberInterface {
  ...
  protected function alterPaginationCommands(&$commands) {
    foreach ($commands as $delta => &$command) {
      ...
      if (isset($command['method']) && $command['method'] === 'replaceWith') {
        $command['method'] = 'infiniteScrollPagerInsertView';
      }
  ...
  public function onResponse(FilterResponseEvent $event) {
    ...
    if ($view->getPager()->getPluginId() !== 'infinite_scroll_with_pager' || $view->getCurrentPage() === 0) {
      return;
    }

    if ($event->getRequest()->request->has('next')) {
